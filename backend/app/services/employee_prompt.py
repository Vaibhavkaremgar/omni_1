...
def _language_conversation_guidance(language: str) -> str:
    ...
    if _is_telugu(normalized):
        ...
        fillers += " SCRIPT RULE: write Hindi words in Devanagari and use English terms only where natural for this call's actual subject. Never write Hindi in Roman letters."
...
def natural_voice_conversation_behavior() -> str:
    return (
        ...
        "Before speaking each turn, silently check the caller's latest words, the conversation history, facts already collected, questions already answered, and script lines already delivered. Respond to the caller's latest meaning first, then choose only the next unfinished step. "
        "Treat the call script as a flexible guide, never a playlist or loop. Do not replay a line, question, explanation, offer, closing, or completed section. Repeat only when the caller explicitly asks; then paraphrase briefly unless an identifier must remain exact. "
