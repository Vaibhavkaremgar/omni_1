...
def _language_conversation_guidance(language: str) -> str:
    ...
    if normalized in {"telugu", "te", "te-in", "telugu (india)"}:
        ...
        hello = "If the caller is silent for approximately 2–3 seconds, say exactly 'Vinipisthundha andi?' (or a natural equivalent) to check that they are present, then continue without ending the call. For repeated hello or attention-seeking, acknowledge with varied Telugu responses such as 'Hello andi, vinipisthunda?' or 'Avunu andi, nenu vintunnanu.'"
        ...
        hello = "If the caller is silent for approximately 2–3 seconds, say 'Kya aap wahan hain ji?' or 'Aap sun rahe hain ji?' to check that they are present, then continue without ending the call. For repeated hello, use varied natural Indian Hinglish such as 'Hello ji, meri awaaz sunai de rahi hai?' or 'Haan ji, main sun raha hoon, aap boliye'. Do not repeatedly restart the welcome and never use Telugu fillers."
    else:
