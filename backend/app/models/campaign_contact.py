...
class CampaignContact(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    provider_bulk_call_id: Mapped[str | None] = mapped_column(String(255), index=True, nullable=True)
    provider_line_id: Mapped[str | None] = mapped_column(String(255), index=True, nullable=True)
    ...
    polling_started_at: Mapped[datetime | None] = mapped_column(nullable=True)
    status_checked_at: Mapped[datetime | None] = mapped_column(nullable=True)
