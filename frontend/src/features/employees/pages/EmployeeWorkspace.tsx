...
export default function EmployeeWorkspace() {
  ...
  return <main className="flex-1 overflow-y-auto bg-slate-50"><div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-10">
    ...
    {(config.script_source === 'assembled' || needsSpokenReview) && <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900"><span>This is a draft outline, not a finished call script. Regenerate it, or write a spoken example in every section before publishing.</span><button onClick={() => void retryGeneration()} disabled={busy} className="rounded-lg bg-amber-900 px-3 py-2 font-semibold text-white disabled:opacity-40">Retry generation</button></div>}
