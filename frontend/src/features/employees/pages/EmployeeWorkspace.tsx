...
type ConversationSection = { title: string; purpose: string; instructions: string; questions: string[]; examples: string[]; handling: string };
export type Config = { [key: string]: unknown; opening?: string; final_prompt?: string; conversation_sections?: ConversationSection[]; website_url?: string; call_script?: Record<string, string>; custom_sections?: Step[]; conversation_variables?: Variable[] };
...
type ScriptValidationIssue = { section: string; type: string; message: string };
type ScriptValidation = { valid: boolean; language: string; issues: ScriptValidationIssue[] };
type GenerationWarning = { section?: string; rule?: string; reason?: string };
...
export default function EmployeeWorkspace() {
  ...
  const [destination, setDestination] = useState('+91');
  ...
  const sixSectionPrompt = sectionNames.map((section, index) => `${index + 1}. ${section}\n${script[section] ?? ''}`).join('\n\n');
  const scriptValidation = (config.script_language_validation as ScriptValidation | undefined) ?? null;
  const generationWarnings = (config.script_generation_warnings as GenerationWarning[] | undefined) ?? [];
  const reviewWarnings = [
    ...(scriptValidation && !scriptValidation.valid ? scriptValidation.issues.map(issue => issue.message) : []),
    ...generationWarnings.map(issue => issue.reason ?? issue.rule ?? 'Review the generated script.'),
  ...
  const save = async (publish = false) => {
    ...
    try {
      ...
      const updated = await backendJson<Employee>(`/employees/${id}`, { method: 'PATCH', body: JSON.stringify({ name: name.trim(), language, call_type: callType, configuration: next }) });
      ...
      const payload = (e as Error & { payload?: { validation?: ScriptValidation } }).payload;
      if (payload?.validation) setConfig(current => ({ ...current, script_language_validation: payload.validation }));
  ...
  const retryGeneration = async () => { setBusy(true); setError(''); setNotice(''); try { const result = await backendJson<{ configuration?: Config }>(`/employees/${id}/generate-script`, { method: 'POST' }); setConfig(result.configuration ?? {}); setNotice('Script generation retried. Review the opening and all six sections.'); } catch (e) { setError(e instanceof Error ? e.message : 'Script generation failed.'); } finally { setBusy(false); } };
